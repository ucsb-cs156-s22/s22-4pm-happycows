...
describe("CommonsCardVisit tests", () => {
    ...
    test("renders without crashing when button text is set", async () => {
        apiCurrentUserFixtures.userOnly.user.root = userCommonsFixtures.oneUserCommons[0];
        ...
        const click = jest.fn();
        render(
            <CommonsCardVisit commons = {commonsFixtures.threeCommons[0]} buttonText = {"Join"} user = {apiCurrentUserFixtures.userOnly.user} buttonLink = {click} buttonText1 = {"Unjoin"} />
        );
    ...
    test("Calls the callback when you click unjoin", async () => {
        ...
        axiosMock.onDelete("/api/commons/1/users/1").reply(200, commonsFixtures.threeCommons[0]);
        ...
        expect(await screen.findByTestId("commonsCardVisit-button-Unjoin-1")).toBeInTheDocument();
        const unjoinButton = screen.getByTestId("commonsCard-button-Unjoin-1");
